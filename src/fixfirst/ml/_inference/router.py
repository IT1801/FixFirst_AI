...
def route_review(
    ...
    try:
        ...
        # 2. FINE-TUNED PREDICTION WITH SENTIMENT FALLBACK
        predicted_features = category_predicted_labels(cat_probs, cat_label_names)
        
        # LAZY CACHE: Only call the LLM once per review, even if multiple sentiments fail
        cached_llm_aspects = None 
        ...
        for feature_key in predicted_features:
            ...
            if sentiment_needs_llm_fallback(sent_probs):
                # Only execute the heavy LLM function if we haven't already done it for this review
                if cached_llm_aspects is None:
                    cached_llm_aspects = label_review_fn(review_text, taxonomy)
                
                llm_match = None
                if cached_llm_aspects:
                    llm_match = next((a for a in cached_llm_aspects if a["feature_key"] == feature_key), None)
                ...
                # Confident Sentiment -> Use Fine-tuned
                results.append(
                    {
                        "feature_key": feature_key,
                        "sentiment": sentiment_predicted_label(sent_probs, sent_label_names),
