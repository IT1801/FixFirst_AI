...
NOTE ON TESTING: dataset construction (dataset.py) and metrics computation
(metrics.py) are independently unit-tested with pure numpy/pandas — no
torch dependency, all tests passing. This training loop follows standard
HuggingFace Trainer conventions for sentence-pair classification but has
NOT been executed end-to-end in this environment (no torch/transformers
install here — see requirements-training.txt). Smoke test with --limit
before a full run.
"""
...
from fixfirst.models.aspect_sentiment.dataset import (
    AspectSentimentDataset,
    SENTIMENT_LABELS,
    build_sentiment_examples,
)
from fixfirst.models.aspect_sentiment.metrics import compute_sentiment_metrics

BASE_MODEL_NAME = "distilbert-base-uncased"  # swap to "microsoft/deberta-v3-base" for higher accuracy
MAX_LENGTH = 128
BATCH_SIZE = 16
NUM_EPOCHS = 4
LEARNING_RATE = 2e-5
...
def train_aspect_sentiment_model(limit: int = None) -> Dict:
    ...
    try:
        ...
        with mlflow.start_run(run_name="aspect_sentiment_classifier") as run:
            ...
            meta = {
                "base_model": BASE_MODEL_NAME,
                "sentiment_labels": SENTIMENT_LABELS,  # index i -> SENTIMENT_LABELS[i]
                "max_length": MAX_LENGTH,
...
def _register_model_run(mlflow_run_id: str, metrics: Dict) -> None:
    ...
    from fixfirst.db.models import ModelRun, ModelTask
